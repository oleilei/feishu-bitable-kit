...
// @require      https://oss.techclub.plus/dict/1.0.5/feishu-bitable.min.js
...
(function () {
  ...
  async function saveImgZip(mainImgArr, detailImgArr) {
    ...
    const upc = goods_info["UPC"];
    if (!goods_code && !upc) {
      debugLog("错误：商品编码和UPC都不存在，尝试刷新页面重试");
      // 检查是否已经重试过
    ...
    const file_name = (upc || goods_code) + "_" + goods_name.replace(/\//g, "-");
    ...
    try {
      ...
      images.forEach(img => {
        ...
        const fileName = `${upc || goods_code}-${img.index + 1}${suffix}`;
  ...
  async function saveToFeishu(goods_info) {
    try {
      ...
      // 存入飞书表格
      const result = await bitable.addRecord(goods_info);
